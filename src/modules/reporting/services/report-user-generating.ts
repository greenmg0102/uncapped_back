...
export class ReportUserGeneratingService {
  ...
  async squeezeUserDataGenerating(body: any) {
    ...
    let buffer = squeezeActionRecursive.find((item: any) => item.squeeze === body.squeeze && item.squeezeAction === body.squeezeAction && item.position === body.heroPosition[0]).rangeList
    ...
    let pipeLine = {
      ...
      "reportDetail.action": {
        $elemMatch: {
          category: { $in: [action] },
          villain: { $in: villianPosiotionList.length > 0 ? villianPosiotionList : [] }
        }
      },
      pokerRoomId: body.pokerType,
      maxTableSeats: body.tableSize,
