...
export function Bet5Parsing(body: any) {

  let max8 = ["UTG", "UTG+1", "LJ", "HJ", "CO", "BTN", "SB", "BB"]
  let bbData = [10, 15, 20, 25, 30, 398750, 50, 60, 80, 100]
  let bufferResult: any = []
  let real: any = []
  let heroPositionList = exchangeIntoNumberFromPositionString(body.heroPositionList.length === 0 ? max8.slice(3, max8.length) : body.heroPositionList)
  ...
  stackDepthList.forEach((stack: any) => heroPositionList.forEach((each: any) => {
    let buffer = RecursiveBet5Parsing(stack, tableSize, each, 0)
    real = buffer
  }));
...
export function RecursiveBet5Parsing(stackDepth: number, maxPlayer: number, destinationPosition: number, currentPosition: number) {

  let result = []

  for (let j = 0; j < destinationPosition - 5; j++) {
    for (let i = j; i < destinationPosition; i++) {
      let buffer = RecursiveBet5ParsingRecursive(stackDepth, maxPlayer, destinationPosition, currentPosition, i, 0)
      result.push(buffer)
    }
  }
...
export function RecursiveBet5ParsingRecursive(stackDepth: number, maxPlayer: number, destinationPosition: number, currentPosition: number, raisePosition: any, sequence: any) {
  ...
  else return RecursiveBet5ParsingRecursive(stackDepth, maxPlayer, destinationPosition, nextNode, raisePosition, sequence + 1)
