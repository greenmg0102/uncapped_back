...
export function Bet4Parsing(body: any) {
  ...
  stackDepthList.forEach((stack: any) => heroPositionList.forEach((each: any) => {
    let buffer = RecursiveBet4Parsing(stack, tableSize, each, 0)
    real = buffer
  }));
...
export function RecursiveBet4Parsing(stackDepth: number, maxPlayer: number, destinationPosition: number, currentPosition: number) {

  let result = []

  for (let j = 0; j < destinationPosition - 4; j++) {
    for (let i = j; i < destinationPosition; i++) {
      let buffer = RecursiveBet4ParsingRecursive(stackDepth, maxPlayer, destinationPosition, currentPosition, i, 0)
      result.push(buffer)
    }
  }
...
export function RecursiveBet4ParsingRecursive(stackDepth: number, maxPlayer: number, destinationPosition: number, currentPosition: number, raisePosition: any, sequence: any) {
  ...
  else return RecursiveBet4ParsingRecursive(stackDepth, maxPlayer, destinationPosition, nextNode, raisePosition, sequence + 1)
